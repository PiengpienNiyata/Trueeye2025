<template>
  <div class="pt-6 flex gap-4">
    <input
      type="text"
      name=""
      id=""
      class="w-[268px] pl-2 h-10 text-[#A4A4A4] flex items-center border border-[#A4A4A4] rounded"
      placeholder="ค้นหา"
    />
    <select
      class="w-[268px] text-[#A4A4A4] flex border border-[#A4A4A4] rounded margin-righ"
    >
      <option>สิทธิ์การใช้งาน</option>
      <option>2023</option>
    </select>
    <button
      class="w-[124px] h-10 flex text-[#D6D6D6] justify-center items-center gap-x-2 border border-[#D6D6D6] rounded"
    >
      <svg width="16" height="18" viewBox="0 0 16 18" xmlns="http://www.w3.org/2000/svg">
        <path
          d="M3 18C2.45 18 1.97933 17.8043 1.588 17.413C1.19667 17.0217 1.00067 16.5507 1 16V3H0V1H5V0H11V1H16V3H15V16C15 16.55 14.8043 17.021 14.413 17.413C14.0217 17.805 13.5507 18.0007 13 18H3ZM13 3H3V16H13V3ZM5 14H7V5H5V14ZM9 14H11V5H9V14Z"
          fill="#D6D6D6"
        />
      </svg>
      <span>รีเซ็ต</span>
    </button>
  </div>
</template>
